...
class Users{
  ...
  public function userRegistration($data){
    ...
    $checkEmail = $this->checkUserEmailExists($email);

    if ($name == "" || $username == "" || $email == "" || $index_number == "" || $password == "") {
...
<strong>Error !</strong> Username is too short, at least 3 Characters !</div>';
        return $msg;
    }elseif (filter_var($index_number,FILTER_SANITIZE_NUMBER_INT) == FALSE) {
      $msg = '<div class="alert alert-danger alert-dismissible mt-3" id="flash-msg">
<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
...
<strong>Error !</strong> Email already Exists, please try another Email... !</div>';
      ...
      $sql = "INSERT INTO tbl_users(name, username, email, password, mobile, type) VALUES(:name, :username, :email, :password, :mobile, :type)";
      $stmt = $this->db->pdo->prepare($sql);
      $stmt->bindValue(':name', $name);
      $stmt->bindValue(':username', $username);
      $stmt->bindValue(':email', $email);
      $stmt->bindValue(':password', SHA1($password));
      $stmt->bindValue(':mobile', $index_number);
  ...
  public function addNewUserByAdmin($data){
    ...
    if ($first_name == "" || $other_names == "" || $email == "" || $password == "") {
      $msg = '<div class="alert alert-danger alert-dismissible mt-3" id="flash-msg">
<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
<strong>Error !</strong> Input fields must not be Empty !</div>';
        return $msg;
    }elseif (strlen($other_names) < 3) {
      $msg = '<div class="alert alert-danger alert-dismissible mt-3" id="flash-msg">
<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
<strong>Error !</strong> Username is too short, at least 3 Characters !</div>';
        return $msg;
...
<strong>Error !</strong> Invalid email address !</div>';
        ...
        $sql = "INSERT INTO users(first_name, other_names, email, password, index_number, type, profile_pic) VALUES(:first_name, :other_names, :email, :password, :index_number, :type, :profile_pic)";
        $stmt = $this->db->alt_pdo->prepare($sql);
        $stmt->bindValue(':first_name', $first_name);
        $stmt->bindValue(':other_names', $other_names);
        $stmt->bindValue(':email', $email);
        $stmt->bindValue(':password', password_hash($password, PASSWORD_DEFAULT));
        $stmt->bindValue(':index_number', $index_number);
        $stmt->bindValue(':type', $type);
        $stmt->bindValue(':profile_pic', "profile.jpg");
        $result = $stmt->execute();
...
//    TODO::  Change this to get from the other database
  ...
  public function CheckActiveUser($email){
    ...
    public function userLoginAuthotication($data){
      ...
      $checkEmail = $this->checkAdminEmail($email);
  ...
  //   Get Single User Information By Id Method
    public function updateUserByIdInfo($userid, $data){
      ...
      if ($first_name == "" || $other_names == ""|| $email == "" ) {
        $msg = '<div class="alert alert-danger alert-dismissible mt-3" id="flash-msg">
    <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
    <strong>Error !</strong> Input Fields must not be Empty !</div>';
          return $msg;
    ...
    <strong>Error !</strong> Username is too short, at least 3 Characters !</div>';
            return $msg;
    //        }elseif (filter_var($index_number,FILTER_SANITIZE_NUMBER_INT) == FALSE) {
        }elseif ($index_number == "" && $type == "STUDENT") {
          $msg = '<div class="alert alert-danger alert-dismissible mt-3" id="flash-msg">
            <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
            <strong>Error !</strong> Enter index number for student!</div>';
            return $msg;
    ...
    public function deleteUserById($remove){
        $query = $this->db->alt_pdo->prepare("DELETE FROM forums WHERE user_id = :user_id ");
        $query->bindValue(':user_id', $remove);
        $query->execute();

        $query = $this->db->alt_pdo->prepare("DELETE FROM replies WHERE user_id = :user_id ");
        $query->bindValue(':user_id', $remove);
        $query->execute();

        $query = $this->db->alt_pdo->prepare("DELETE FROM subscriptions WHERE user_id = :user_id ");
        $query->bindValue(':user_id', $remove);
        $query->execute();

        $query = $this->db->alt_pdo->prepare("DELETE FROM topics WHERE user_id = :user_id ");
        $query->bindValue(':user_id', $remove);
        $query->execute();
